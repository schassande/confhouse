...
Exception: the sponsor manager notification is template-only. If its Mailjet template identifier is missing or unusable, the backend logs the configuration issue and skips the notification without a server-rendered fallback.
...
### Sponsor Manager Notification Variables

The sponsor manager notification is sent automatically when a new sponsor is created. It is addressed to `Conference.sponsoring.email` and uses `SponsorType.templateEmail.emailManagerNotificationTemplateId`.

The backend provides these variables:
...
`sponsorAdminUrl` is built from the server `ADMIN_BASE_URL` environment variable and the sponsor administration route.
...
- manager notification: sponsor manager notification variables
